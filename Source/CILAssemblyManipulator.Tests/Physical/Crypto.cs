...
namespace CILAssemblyManipulator.Tests.Physical
{
   public class CryptoTests : AbstractCAMTest
   {
         ...
         )
      {
         ...
         {
            var dotNetParams = RSAParams.CreateDotNETParameters();
         ...
         var manualSigCAM = manualCAM.ToByteArray( BinaryEndianness.LittleEndian, includeSign: false );
      ...
      {
         Byte[] camSignature;
         using ( var camCrypto = cryptoFactory() )
         {
            // Don't use rParams.CreateCAMParameters() since the rsa.ExportParameters() already did the LE -> BE conversion
            camSignature = camCrypto.CreateSignature(
               hash,
               new RSAKeyBLOBParsingResult( RSAParams, null, CILAssemblyManipulator.Physical.AssemblyHashAlgorithm.SHA1 ),
               null
               );
            // The native signature is in BE format, reverse CAM signature.
            Array.Reverse( camSignature );
         }
      ...
      private static BigInteger ManualRSA_CAM( Byte[] inputData )
      {
         ...
         var p = BigInteger.ParseFromBinary( parameters.P, BinaryEndianness.BigEndian, 1 );
         var q = BigInteger.ParseFromBinary( parameters.Q, BinaryEndianness.BigEndian, 1 );
         var dp = BigInteger.ParseFromBinary( parameters.DP, BinaryEndianness.BigEndian, 1 );
         var dq = BigInteger.ParseFromBinary( parameters.DQ, BinaryEndianness.BigEndian, 1 );
         var iq = BigInteger.ParseFromBinary( parameters.InverseQ, BinaryEndianness.BigEndian, 1 );
         var input = BigInteger.ParseFromBinary( inputData, BinaryEndianness.BigEndian, 1 );
         BigInteger retVal;
         //if ( p.HasValue && q.HasValue && dp.HasValue && dq.HasValue )
         //{
         // Decryption
